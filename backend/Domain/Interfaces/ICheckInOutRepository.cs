...
public interface ICheckInOutRepository : IRepository<Entities.Checkinout, int>
{
    ...
    Task<Entities.Checkinout?> GetActiveCheckInByCardAsync(int cardId, CancellationToken ct = default);
    Task<Entities.Checkinout?> GetActiveCheckInByMemberAsync(int memberId, CancellationToken ct = default);
    ...
    Task<string> ExecuteAutoCheckoutAsync(CancellationToken ct = default);
}
