...
{
    private const string GroupScheduleType = "G";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (request.CourseDate is null)
        {
            return (false, null, "请选择上课日期。");
        }
        ...
        // 同一会员同一团课同一上课日不可重复预约；不同周次允许
        var activeSameDay = await _bookingRepository.GetActiveByMemberCourseAndDateAsync(
            request.MemberId,
            request.CourseId,
            targetDate,
            cancellationToken);
        if (activeSameDay is not null)
        {
            return (false, null, "您已预约该日的此团课");
        }
        ...
        await EnsureGroupMemberScheduleAsync(
            bookingId,
            request.MemberId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var startTime = ResolveBookingStartTime(booking);
        ...
        _groupPackageRepository.Update(trackedPackage);
        _groupcourseRepository.Update(trackedCourse);
        await CancelGroupMemberScheduleAsync(booking.BookingId, cancellationToken);
    ...
    private async Task EnsureGroupMemberScheduleAsync(
        int bookingId,
        int memberId,
        ...
        CancellationToken cancellationToken)
    {
        var existing = await _memberScheduleRepository.GetBySourceTrackedAsync(
            GroupScheduleType,
            bookingId,
            cancellationToken);
        ...
        existing.ScheduleDate = scheduleDate.Date;
        existing.ScheduleStart = start;
        existing.ScheduleEnd = end;
        existing.Status = "0";
    }

    private async Task CancelGroupMemberScheduleAsync(int bookingId, CancellationToken cancellationToken)
    {
        var schedule = await _memberScheduleRepository.GetBySourceTrackedAsync(
            GroupScheduleType,
            bookingId,
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        while (true)
        {
            ...
            var courseWithSlots = (await _groupcourseRepository.GetAllAsync(cancellationToken))
                ...
                ?? course;
            var nextSlot = courseWithSlots.TimeSlot?.TimeSlotInstances
            ...
            await _bookingRepository.AddAsync(new GroupCourseBooking
            {
                ...
                BookingTime = DateTime.Now,
                BookingStatus = "1",
                CourseDate = nextSlot?.CourseDate.Date,
            }, cancellationToken);
    ...
    private static GroupCourseBookingDto MapBookingEntity(GroupCourseBooking booking, DateTime now)
    {
        ...
        var start = ResolveBookingStartTime(booking);
        ...
        return new GroupCourseBookingDto
        {
            ...
            CourseDate = booking.CourseDate?.Date,
    ...
    private static DateTime? ResolveBookingStartTime(GroupCourseBooking booking)
    {
        var course = booking.Package?.Course;
        if (course is null)
        {
        ...
        if (booking.CourseDate is not null)
        {
            var matched = course.TimeSlot?.TimeSlotInstances
                .FirstOrDefault(x => x.CourseDate.Date == booking.CourseDate.Value.Date);
            if (matched is not null)
            {
                return matched.StartTime;
            }
        }

        return ResolveCourseStartTime(course);
    }
