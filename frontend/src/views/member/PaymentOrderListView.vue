...
async function handleCancel(order: PaymentOrder) {
  if (!canCancel(order)) {
    errorMessage.value = '仅待支付订单可以取消。'
    return
  }

  const no = displayOrderNo(order)
  if (!window.confirm(`确认取消待支付订单 #${no}？`)) return
...
<template>
  <div class="payment-orders">
    ...
    <StateCard v-else-if="orders.length === 0" :message="isAdmin ? '未找到匹配订单。' : '暂无订单。可先去「购买会员卡」下单，或从团课/私教等业务页完成下单后再来支付。'" />
