...
<template>
  <div class="group-booking-page">
    ...
    <template v-else>
      ...
      <section v-if="waitingQueues.length" class="panel">
        <h2>候补中</h2>
      ...
      <section v-if="absenceRecords.length" class="panel">
        <h2>爽约记录</h2>
...
.ghost-link {
  display: inline-flex; padding: 10px 16px; border-radius: 999px; border: 1px solid #d7e0ef;
  color: #2a3c59; text-decoration: none; font-weight: 600;
}
.notice { padding: 12px 16px; border-radius: 10px; margin-bottom: 16px; font-weight: 600; }
.notice.success { background: #e8f7ee; color: #15803d; }
.notice.error { background: #fcebed; color: #dc2626; }
.panel {
  margin-bottom: 20px; padding: 20px; border-radius: var(--tj-radius);
  background: var(--tj-card-bg); box-shadow: var(--tj-shadow);
}
.panel h2 { margin: 0 0 14px; }
.pkg-list, .booking-list { display: grid; gap: 12px; }
.pkg-card, .booking-card { padding: 14px; border-radius: 12px; background: #f8fafc; }
.pkg-card.usable { outline: 1px solid #86efac; }
...
.book-form select, .book-form button, .booking-card button {
  padding: 10px 12px; border-radius: 10px; border: 1px solid #d7e0ef;
}
.book-form button, .booking-card button {
  border: none; background: #4d77ff; color: #fff; font-weight: 600; cursor: pointer;
}
