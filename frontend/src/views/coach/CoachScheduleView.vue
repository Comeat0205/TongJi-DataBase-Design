...
function blockStyle(item: CoachScheduleItem) {
...
<template>
  <div class="coach-schedule">
    <PageHeader
      ...
      title="教练日程"
      subtitle="按周课表查看授课安排；可切换星期。已过日期整列置灰（含当天已上完的课）；空档时间轴自动收缩。"
    >
      <template #actions>
          ...
          >
            ...
            >
              <div class="block-badges">
                ...
                <span v-if="item.isConflict" class="badge conflict-b">冲突</span>
                <span class="badge status">{{ formatStatus(item.status) }}</span>
              </div>
              <strong>{{ courseTitle(item) }}</strong>
              <span>会员：{{ memberLabel(item) }}</span>
              <span>
...
.dot.conflict {
  ...
  box-shadow: inset 0 0 0 1px rgba(192, 57, 43, 0.8);
}
