...
async function handleBooking(booking: PtBooking, accept: boolean) {
  ...
  try {
    ...
    successMessage.value = accept
      ? `已确认 ${booking.courseName} 预约，已写入双方日程。`
      : `已拒绝 ${booking.courseName} 预约，已返还会员 1 次课包次数。`
    await loadBookings()
  } catch (error) {
...
<template>
  <div>
    ...
    <section v-else class="confirmation-list panel-tone-blue">
      <article v-for="item in bookings" :key="item.ptBookingId" class="confirmation-card list-item">
