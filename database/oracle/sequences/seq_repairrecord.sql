-- I · 报修记录主键序列
-- 仅在序列不存在时创建；首次值从当前 REPAIRRECORD 最大主键之后开始。
DECLARE
...
BEGIN
    ...
    FROM USER_SEQUENCES
    WHERE SEQUENCE_NAME = 'SEQ_REPAIRRECORD';

    IF v_sequence_count = 0 THEN
        SELECT NVL(MAX(RECORD_ID), 0) + 1
        INTO v_start_with
        FROM REPAIRRECORD;

        IF v_start_with > 9999999999 THEN
            RAISE_APPLICATION_ERROR(-20001, 'REPAIRRECORD 主键已超出 NUMBER(10) 范围');
        END IF;

        EXECUTE IMMEDIATE
            'CREATE SEQUENCE SEQ_REPAIRRECORD ' ||
